...
def tiles(filename):
    # data/ also holds the points parquet, so serve only the configured
    # basemap. conditional=True enables the HTTP range requests the pmtiles
    # protocol relies on to read slices of the archive.
...
def api_points():
    ...
    limit = request.args.get("limit", default=config.POINTS_MAX_FEATURES, type=int)
    geojson, n, capped = points.query_bbox(minx, miny, maxx, maxy, limit)

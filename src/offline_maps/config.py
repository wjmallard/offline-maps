...
# Point dataset (GeoParquet)
POINTS_PARQUET = (_ROOT / _config["points"]["file"]).expanduser()
POINTS_MAX_FEATURES = _config["points"].get("max_features", 5000)

# ALPR sync source — the DeFlock CDN base URL for the region-tile mirror (see sync.py)
...
SYNC_OSM_OUTPUT = (_ROOT / _sync_osm.get("output_path", "data/points_meta.parquet")).expanduser()

# Optional local photo cache (see sync_photos.py); MAPILLARY_TOKEN comes from .env
_sync_photos = _sync.get("photos", {})
SYNC_PHOTOS_DIR = (_ROOT / _sync_photos.get("dir", "data/photos")).expanduser()
SYNC_PHOTOS_THUMBS_DIR = SYNC_PHOTOS_DIR / "thumbs"
